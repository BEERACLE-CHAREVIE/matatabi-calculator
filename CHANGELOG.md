...
本プロジェクトの主要な変更を記録します。形式は [Keep a Changelog](https://keepachangelog.com/ja/1.1.0/) をベースに、プロジェクト独自カテゴリ（`Verified` / `Notes for Sales / Product Team` / `Future Work`）を商談・運用ハンドオフ用途で追加しています。バージョニングは [Semantic Versioning](https://semver.org/lang/ja/) に従います。

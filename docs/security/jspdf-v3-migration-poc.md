...
> ステータス: 調査中（2026-05-01 時点の暫定判定）
> 関連 Issue: #24（本書の起票元、判断項目 (b)） / #5（PDF レポート実装、本書の判定を待つ） / #8（依存ライブラリ追加、`^2.5.2` 採用の経緯）
...
**v3 / v4 への即時移行は推奨しない**。理由:

1. `docs/security/jspdf-vulnerabilities.md §3` の到達経路分析で 10 件すべて到達不能と判定済み。
2. v3 / v4 系のリリースから Issue #5 着手予定までの期間が短く、「枯れ」が不足している。
3. `html2canvas` との連携破壊リスクが Issue #5 のスケジュールと正面衝突する。

### 5.2 本 Issue でのアクション

**コードを書かない**。本 Issue は意思決定ドキュメントの追加に留め、実機 PoC は Issue #5 着手時点に持ち越す。

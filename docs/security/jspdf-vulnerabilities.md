...
- **除外対象 GHSA-ID リスト**（`scripts/audit-check.mjs` の `ALLOWED_GHSA_IDS` と一致）:
  ...
  - `GHSA-9vjf-qc39-jprp` / `GHSA-cjw8-79x6-5cj4` — §2.4（`addJS` 系、`addJS` API 未使用。なお後者は moderate のため severity フィルタで自動除外されるが、文書追跡のため §1.1 に併記）
  ...
  - dompurify 系 7 件（§1 #4〜#10）は moderate のため severity フィルタで自動除外される（明示的な allowlist 登録は不要）。§2.3 で到達不能と判定済み。
- **採用しなかった選択肢**:

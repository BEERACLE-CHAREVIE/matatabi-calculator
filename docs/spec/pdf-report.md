...
| 指標カード幅（3 枚並列） | 各 56mm、カード間 5mm | 描画領域 180mm = 56×3 + 5×2 で全幅一致（Issue #85 でカード間を 6mm → 5mm に圧縮、value 1 行収納の自動縮小と組み合わせ） |
| 指標カードの高さ | **`min-height: 34mm`、value 桁数に応じて自動伸長**（Issue #85） | 固定 `height: 34mm` だと value 桁爆発時に note と縦衝突。`gridAutoRows: minmax(34mm, auto)` + 値長応答のフォントサイズ縮小で吸収 |
...
| `LabelList` の `formatter` | 比率 5% 未満で空文字（旧） | **比率 8% 未満で空文字 / 8〜18% で `formatManYenCompact` / 18% 以上で `formatManYen` の段階降格**（Issue #85） | scale=2 ラスタライズで insideLeft / insideRight が衝突する事象に対し、SVG `<text>` は `text-overflow` が効かないため formatter 段階で吸収する。 |
| `Legend` の配置 | Recharts デフォルト（絶対配置） | **`verticalAlign='bottom'` + `wrapperStyle.position='relative'` + `marginTop: 2mm`**（Issue #85） | 絶対配置で chart 領域と Legend が重なる事象を抑止。アイコンと文字の縦ズレは `alignItems: baseline` + 明示 `lineHeight` で固定。 |
...
- [x] 指標カードは min-height + 自動伸長運用に切替（Issue #85, §5.3 / §9.2）

...
| 指標カード幅（3 枚並列） | 各 56mm、カード間 6mm | 描画領域 180mm = 56×3 + 6×2 で全幅一致 |
...
| `Legend` の `wrapperStyle` | `position: absolute`（Recharts デフォルト） | `position: "relative"` + `verticalAlign="bottom"`、`<li>` を `alignItems: "baseline"` に固定 | chart 領域との重なりとアイコン↔テキストの縦ずれを抑止（Issue #85）。 |

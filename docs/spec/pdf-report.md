...
- `CalculationResult` 型（[`calculation-logic.md §5`](./calculation-logic.md)）— `threeYearSavings` / `annualProfitCreation` / `threeYearProfitCreation` / `totalThreeYearImpact` / `speedWarning` / `insourcingGap`。追加フィールド `speedWarningMonthlyLoss: number`（円単位、`monthlyVendorCost * insourcingGap`）は [`warning-copy.md §7.3, §11.1`](./warning-copy.md) で `calculation-logic.md §5` への追記を要請済みのフォローアップ項目。本書はこの追記完了を前提として参照する。
...
- 入力 5 項目 + 詳細設定 3 項目（[`input-form.md §3.3, §4.1〜§4.4`](./input-form.md)）— PDF 入力サマリーの表示対象。`updateWaitMonths` は離散 5 段階セレクト（代表値）。
...
- 警告バナー確定文言（[`warning-copy.md §3, §7.4, §9.2`](./warning-copy.md)）— `CRITICAL OPPORTUNITY LOSS` + 日本語サブ、動的数字差し込み `buildCriticalOpportunityLossMessage(monthlyLossYen)`（戻り値 `{ headline: string; subtext: string }`）、`speedWarning && insourcingLevel !== 1` で発動。
...
| # | 要素 | 出典 | 必須度 | 高さ |
|---|---|---|---|---|
| 1 | ヘッダー（ロゴ + サービス名 + 生成日時） | 本書 §6.1 | 必須 | 18mm |
| 2 | 警告バナー（`CRITICAL OPPORTUNITY LOSS`） | [`warning-copy.md §3`](./warning-copy.md) | 条件付き必須 | 14mm |
| 3 | 表紙訴求（3 年トータルインパクト、ヒーロー数値） | [`calculation-logic.md §6.1`](./calculation-logic.md) | 必須 | 42mm |
| 4 | 指標カード（3 年止血・年間創出・3 年創出の並列） | [`result-dashboard.md §3.2`](./result-dashboard.md) | 必須 | 38mm（Issue #85 で 34mm → 38mm に拡張、フォント line-height clip 防止） |
...
| 7 | カテゴリ別訴求テキスト | [`calculation-logic.md §4.4`](./calculation-logic.md) | 推奨 | 14mm |
| 8 | 入力サマリー（基本 5 項目 + 詳細設定） | [`input-form.md §3.3`](./input-form.md) | 必須 | 40mm |
| 9 | 免責注記 | [`calculation-logic.md §6.1`](./calculation-logic.md) / 本書 | 必須 | 8mm |
| 10 | フッター（会社名・連絡先・ページ番号） | 本書 §6.2 | 必須 | 13mm |

**高さ予算の検算**: 描画領域は A4 縦 297mm からページマージン上下 15mm ずつを引いた **267mm**。

| ケース | 要素高さ合計 | セクション間余白（§5.3 で 3mm） | 合計 | 余裕 |
|---|---|---|---|---|
| 警告発動時（9 セクション、8 区切り） | 229mm（Issue #85 で指標カード +4mm） | 24mm（8 × 3mm、注 1） | **253mm** | 14mm 余裕 |
| 警告非発動時（8 セクション、7 区切り） | 215mm（Issue #85 で指標カード +4mm） | 21mm（7 × 3mm） | **236mm** | 31mm 余裕 |

> 注 1: 要素 6「内製化注記」は要素 4「指標カード」の内部に配置するため、独立したセクションとしてはカウントしない。セクション数は要素 1→2→3→4→5→7→8→9→10 の 9 本、区切り数は 8 本。
...
| セクション間余白 | **3mm** | 全要素共通で統一。§5.2 の高さ予算検算と整合 |
| カード padding | 縦 6mm / 横 5mm | 指標カード内部。34mm のカード高さに合わせて圧縮 |
| ヒーロー数値フォントサイズ | **32pt**（桁爆発時 24pt） | 42mm の表紙訴求枠に収めるサイズ。`formatManYenCompact` による「◯億◯万円」表記が発生した場合に自動縮小 |
| 指標カード value フォントサイズ | **16pt**（`value.length > 8` で 14pt、`> 11` で 13pt に段階縮小） | 桁膨張時のフォールバック。下方 note との衝突を物理的に防ぐ（Issue #85） |
| 警告バナー高さ | **14mm** | 画面版 64〜96px からの圧縮 |
...
| 指標カード幅（3 枚並列） | 各 56mm、カード間 5mm | 描画領域 180mm = 56×3 + 5×2 + 余白 で 1 行幅にマージンを確保（Issue #85 で 6mm → 5mm に圧縮、`formatMetricCardValue` の Compact 自動降格と組み合わせ「100,000万円」級の長文字列でも 1 行収納） |
| ロゴ配置領域 | **12 × 12mm** の正方形 | §6.1 参照。mm 単位で統一（pt 併用はしない） |

> [`warning-copy.md §8.3`](./warning-copy.md) の申し送り「A4 比率でのバナー高さ・フォントサイズ調整」は本表で確定する。
> 指標カードの高さは `min-height: 34mm` の `gridAutoRows: minmax(34mm, auto)` 運用とし、value 桁数に応じて自動伸長する（Issue #85）。最大伸長分は §5.2 の余裕予算（警告発動時で 18mm）に収まる範囲で吸収する。
...
```
┌─────────────────────────────────────────────────────────┐
│ [ロゴ]  ROI診断結果レポート            生成日時 2026-04-23 15:30 │ ← ヘッダー 18mm
├─────────────────────────────────────────────────────────┤
│ ⚠  CRITICAL OPPORTUNITY LOSS                              │
│    現在、月額 120 万円相当の機会損失が発生中              │ ← 警告バナー 14mm
├─────────────────────────────────────────────────────────┤
│                                                           │
│            3年間のトータルインパクト                      │
│               ◯,◯◯◯ 万円                                │ ← 表紙訴求 42mm
...
├─────────────────────────────────────────────────────────┤
│ ◆ 診断カテゴリ                                            │
│   {{CATEGORY_MESSAGE}}                                    │ ← カテゴリ訴求 14mm
├─────────────────────────────────────────────────────────┤
│ ◆ 入力値サマリー                                          │
│  月額ベンダー費用    ◯◯万円／月                          │
│  単発改修費用        ◯◯万円／回                          │ ← 入力サマリー 40mm
│  手作業人数          ◯名                                  │
│  更新待ち期間        ◯〜◯ヶ月 (※)                        │
│  内製化状況          ◯◯◯ (◯%)                           │
│  (詳細設定: 時給 ◯円 / ◯h/日 / ◯日/月)                   │
├─────────────────────────────────────────────────────────┤
│ 本試算は入力値に基づく理論上の最大値であり、...           │ ← 免責 8mm
├─────────────────────────────────────────────────────────┤
│ 株式会社ねこにまたたび  contact@...             1 / 1     │ ← フッター 13mm
└─────────────────────────────────────────────────────────┘
```

> **(※) 更新待ち期間のレンジ表記について**: `updateWaitMonths` は [`input-form.md §4.4`](./input-form.md) で離散 5 段階セレクト（代表値 `0.5 / 1.5 / 4.5 / 9 / 18`）として確定済み。PDF 入力サマリーでは**セレクト選択肢のラベル（例: 「3〜6ヶ月」）を表示**する運用とする（計算時は単一代表値を使用するため `calculation-logic.md` 側との整合は保たれる）。
...
- 警告バナー（14mm + セクション間余白 3mm、合計 17mm）の領域が空く場合、**表紙訴求（ヒーロー）の位置を上に繰り上げる**（`showWarningBanner` フラグで条件分岐）。
...
| 領域 | 内容 | タイポグラフィ |
|---|---|---|
| 左端 | ロゴ（**12 × 12mm** の正方形領域、暫定は文字ロゴ `またたび計算機`） | Inter Bold 14pt |
| 中央 | `ROI診断結果レポート` | Noto Sans JP Bold 14pt |
| 右端 | `生成日時 2026-04-23 15:30`（JST 固定、分単位） | Inter + Noto Sans JP Regular 10pt |

- **高さ**: 18mm（§5.2 予算と一致）。下端に Slate 200 の細線 1px を引いて本文と区切る。ヘッダー周辺寸法は mm で統一し、pt はフォントサイズのみに限定する（可読性確保のため）。
...
- **高さ**: 13mm（§5.2 予算と一致）。上端に Slate 200 の細線 1px。
...
- 本書では「ヘッダー左端 **12 × 12mm** の正方形領域にロゴを配置する」とレイアウト枠のみ確定（§6.1）。ロゴ SVG の制作・配置は **別 Issue（仮: Issue #99 ブランドアセット制作）で対応**する。
...
[PdfDashboard の隠しDOM をマウント (A4 比率固定、position: absolute; left: -9999px)]
    ↓
[requestAnimationFrame 2 回分待機 (DOM レイアウト確定)]
    ↓
[await document.fonts.ready (Web フォント Noto Sans JP / Inter の非同期解決を待機。Issue #85 でフォントスワップ前後の字形重畳による文字被り対策として明示化)]
    ↓
...
| 項目 | 挙動 |
|---|---|
| タイムアウト | **10 秒**。超過時は「PDFの生成に失敗しました。再試行してください。」を表示。 |
| エラー表示 | ボタン直下にインラインメッセージ + **再試行ボタン**を併設。Issue #47 で「5 秒自動消去」から「**ユーザーが再試行 / 再診断を実行するか PDF 生成が成功するまで表示し続ける**」方針へ変更（再試行ボタンが消える前にメッセージが消えると操作が破綻するため）。 |
| リトライ | ユーザーが「再試行」ボタンまたは元の「PDF をダウンロード」ボタンを再度押すことで再試行可能。 |
| 連続失敗エスカレーション | **3 回連続で失敗**したら文言を「PDF の生成に複数回失敗しました。お手数ですが、お問い合わせフォームからご連絡ください。」に切り替え、再試行ボタンに代えて `/contact` への内部リンクを表示する。成功時 / 再診断時にカウンタを 0 リセット。 |
| ログ | `console.error` にエラー詳細を出力（商談中のデバッグに使える）。Sentry / Cloudflare Pages ログ統合は §13.7 将来拡張。 |
| アナリティクス | `trackEvent` で `pdf_generation_error` / `pdf_generation_error_escalated` を送出（Issue #47）。`error.message` には個人情報が混入するリスクがあるためイベント名のみを送る。CF Web Analytics 単独採用中は no-op スタブとして動作する。 |
...
| 項目 | 画面版 | PDF 版 | 根拠 |
|---|---|---|---|
| ヒーロー数値フォントサイズ | `clamp(2.5rem, 6vw, 4rem)`（[`result-dashboard.md §7.2`](./result-dashboard.md)） | **32pt**（桁爆発時 24pt） | 42mm の表紙訴求枠に合わせる。桁爆発時は `formatManYenCompact` との組み合わせで縮小。 |
| 指標カード数 | 2 枚（止血 + 年間/3 年の二段表記） | **3 枚並列**（止血 / 年間 / 3 年） | §5.5 の合理性議論による。 |
| 警告バナー高さ | 64〜96px | **14mm** | A4 密度優先。 |
| セクション間余白 | `space-y-6`（24px） | **3mm** | A4 密度優先。§5.2 の高さ予算検算と整合。 |
...
| `LabelList` の `formatter` | （画面版なし、`<LabelList>` 自体を使用しない） | `pickPdfBarLabel(value, total)` で `total <= 0` ガード + 比率閾値による段階フォールバック（< 8% で空文字、< 18% で `formatManYenCompact`、それ以上で `formatManYen`） | バー内ラベル同士の衝突を formatter 段階で抑制（Issue #85）。 |
| `Legend` の `wrapperStyle` | Recharts デフォルト（`position: absolute`）+ `<li>` は `items-baseline` + `leading-tight` で揃え | `position: "relative"` + `verticalAlign="bottom"` + `marginTop: 2mm`、`<li>` を `alignItems: "baseline"` に固定 | chart 領域との重なりとアイコン↔テキストの縦ずれを画面・PDF 双方で抑止（Issue #85）。 |
...
- 2026-05 時点で `jspdf@^2.5.2` には `npm audit` が critical 1 / high 2 / 推移依存 dompurify moderate 7 の脆弱性を報告するが、すべて本アプリの利用形態（html2canvas 主体・テンプレート生成ファイル名・`jsPDF.html()` 不使用）では到達不能と判定済み。詳細は [`docs/security/jspdf-vulnerabilities.md`](../security/jspdf-vulnerabilities.md) を参照。
- v3 / v4 への移行は Issue #5 着手時点で再評価する。PoC 調査ノート: [`docs/security/jspdf-v3-migration-poc.md`](../security/jspdf-v3-migration-poc.md) 。

### 10.2 html2canvas-pro

| 項目 | 値 |
|---|---|
| 採用パッケージ | **`html2canvas-pro`**（`^2.0.x`） |
| 元 fork 元 | `html2canvas` v1.4.1 |
| TypeScript 型定義 | 公式同梱 |
| ロード戦略 | jsPDF と同じタイミングで dynamic import |

- `src/lib/pdf.ts` 内で `await import('html2canvas-pro')` により動的ロード。
- **`html2canvas` v1.4.1 → `html2canvas-pro` v2 への置換理由（Issue #85）**:
  - `html2canvas` 本家は 2022 年以降メンテナンス停止。`oklch()` カラー / `:has()` /
    modern flexbox / SVG `<text>` の baseline 揃え / Web フォント (`next/font` 経由) の
    解決などで複数のレンダリングバグを抱える。
  - `html2canvas-pro` は同 fork のアクティブメンテナンス版で API ドロップイン互換。
    Issue #85 の文字被り (Web フォントスワップ前後の二重描画) は `document.fonts.ready`
    + flex-shrink 圧縮対策で軽減したが、根本のブラウザレンダリング差異は本ライブラリ
    置換でようやく安定する。
...
>
> **§10.3 構造化フォローアップとの連動**: 上記 JSX の `warningMessage` は [`warning-copy.md §9.3`](./warning-copy.md) で確定した構造体 `{ headline: string; subtext: string }` を渡す。`result-dashboard.md §10.3` の `DashboardViewProps.warningMessage` は [`warning-copy.md §11.2`](./warning-copy.md) で構造化更新が申し送られており、本書はその更新完了を前提とする。構造化更新が完了するまでは `buildCriticalOpportunityLossMessage` の戻り値を一時的に `.subtext` のみ使用する等の暫定対応を Issue #6 実装時に判断する。
...
- [x] ヒーロー数値フォントサイズ 32pt（桁爆発時 24pt）を確定（§5.3）
- [x] 指標カード数（PDF 3 枚並列）と画面版との差分の合理性明記（§5.5）
- [x] 警告バナー高さ 14mm・見出し 12pt・サブ 10pt を確定（§5.3）
- [x] 警告非発動時のレイアウト繰り上げ方針確定（§5.4, §5.6）
- [x] 完全内製顧客でのレイアウト破綻防止（§5.6）
- [x] 指標カードは min-height + 自動伸長運用に切替（Issue #85）
...
- [x] エラーハンドリング（インラインメッセージ + 再試行ボタン、連続失敗 3 回でエスカレーション）確定（§8.3、Issue #47 で自動消去から手動消去に方針変更）

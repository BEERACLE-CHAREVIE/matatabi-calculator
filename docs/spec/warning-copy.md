...
> **確定仕様は [`pdf-report.md`](./pdf-report.md) を参照**。バナー高さ 15mm・見出し 12pt・サブ 10pt に確定（[`pdf-report.md §5.3`](./pdf-report.md)）、生成日時はヘッダー右端に JST 固定で掲出（[`pdf-report.md §6.1`](./pdf-report.md)）、`animated={false}` 強制は [`pdf-report.md §9.1`](./pdf-report.md) で継承確認。

...
- `updateWaitMonths` は離散 5 段階セレクト（[`input-form.md §4.4`](./input-form.md)）— `0.5 / 1.5 / 4.5 / 9 / 18` のうち下 3 値で警告発動。
...
**採用理由**:
- マスター設計書のフレーズを継承し、商品コンセプト（「また旅（またたび）計算機」のキャッチコピーとしての `CRITICAL OPPORTUNITY LOSS`）との連続性を保つ。
...
| e | 現在、月額 `{monthlyLoss}` 万円相当の機会損失が発生中 | 動的 | 数字差し込み（§7） | **採用（既定）** |
...
[`input-form.md §4.4`](./input-form.md) の離散 5 段階セレクトでは、下 3 値（`4.5 / 9 / 18`）で警告が発動する。選択肢ラベル境界（「1〜2ヶ月」と「3〜6ヶ月」の間）が警告境界と一致しており、営業担当が「3 ヶ月以上の選択肢を選ぶと警告が出る」と即説明できる。
...
**不採用案**:
- **3 段階で文言／色を出し分ける**（例: `4.5ヶ月=Amber 300 注意 / 9ヶ月=Amber 500 警告 / 18ヶ月=Red 500 重大`）: Issue #3 §8.2 の改定を誘発。`CalculationResult` 型拡張が必要。実装・テストコスト増。フレーズの誇大問題が発生。
...
- テキスト色: `slate-900`（見出し）、`slate-700`（サブテキスト）

上記は [`result-dashboard.md §8.2`](./result-dashboard.md) を正本とし、本書では再掲のみ。本書による改定なし。
...
| 日本語サブテキスト | 13〜14px | 12〜13px | 13〜14px | 400 | `slate-700` | 1.4〜1.5（スマホ 1.4） |
...
**最長ケース**:
- 英語見出し: `CRITICAL OPPORTUNITY LOSS`（25 文字、1 行）
- 日本語サブ: `現在、月額 1,200 万円相当の機会損失が発生中`（24 文字、1 行）
 ...
 * monthlyLossYen <= 0 のとき: 汎用のフォールバック文言を返す。
 *
 * 呼び出し側は以下の事前条件を満たすこと:
 *   - result.speedWarning === true（= 更新待ち 3 ヶ月以上）
 *   - insourcingLevel !== 1（= 完全内製ではない）
 * ResultDashboard コンテナが上記条件を満たすときのみ本関数を呼ぶ。
 */
...
本書で確定した文言・UI を統合した表示例を以下に示す（擬似ワイヤー）。
...
### 11.2 `result-dashboard.md §8.3 / §8.5 / §10.3` への追記運用

- §8.5 のプレースホルダ `"{{CRITICAL_OPPORTUNITY_LOSS_MESSAGE}}"` 記述を本書（`warning-copy.md`）への参照に置き換え。
...
### フォローアップ（本書外）
- [ ] `calculation-logic.md §5` への `speedWarningMonthlyLoss` 追記 → §11.1
- [ ] `result-dashboard.md §8.3 / §8.5 / §10.3` の追記更新 → §11.2
- [ ] マスター設計書 §3.1 に `warning-copy.md` への 1 行参照を追加
...
2. `docs/spec/result-dashboard.md §8.3` のタブレット 64px を「2 行既定」へ更新、§8.5 のプレースホルダ記述を本書参照に置換、§10.3 の `warningMessage` 型を構造化オブジェクトに更新、§13 関連 Issue テーブルに参照追記。
...
5. 文言の社内承認（§11.5）が完了次第、本書ヘッダの「ドラフト」表示を外して正本化。

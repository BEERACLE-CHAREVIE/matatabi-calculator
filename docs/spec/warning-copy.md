...
- アイコン: Lucide `alert-triangle`、色 `accent` (#9CAEB8、`text-accent`)、サイズ 20×20px
- テキスト色: 見出し `ink` (#72665B、`text-ink`)（[`result-dashboard.md §8.2`](./result-dashboard.md) を継承）、サブテキスト `ink/80`（不透明度 80%、見出しと同色相で階層差を不透明度で表現。**本書 §6.2 で新規確定**）
